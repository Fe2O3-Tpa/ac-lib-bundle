...
### `ac-lib-bundle --bin {bin_target}`

Cargo の `bin` ターゲット名を指定して、そのバンドル結果を標準出力に出力します。

### `ac-lib-bundle {bin_target}`

`{bin_target}` がファイルパスとして存在しない場合は、`bin` ターゲット名として扱います。
...
- `bin` ターゲット名でも、`src/bin/{name}.rs` などの配置から展開対象を解決できる
...
4.5. `--bin` か位置引数の `bin` 名が与えられた場合は、`Cargo.toml` の `[[bin]]` や標準配置から対応する `*.rs` を解決する
...
- `bin` ターゲットは、`[[bin]]` の `name` / `path`、または `src/bin/{name}.rs` / `src/bin/{name}/main.rs` を探します
